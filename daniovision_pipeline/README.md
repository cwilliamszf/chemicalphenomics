...
- `mobile_bout_count`, `mean_mobile_bout_duration_s`, `mean_immobile_bout_duration_s`, `mean_mobile_bout_distance_<unit>`, `mean_immobile_bout_distance_<unit>` -- bout structure of movement, from whichever mobility source applies (same as above). `mean_mobile_bout_distance_<unit>` is the total distance moved during a bout (summed frame-to-frame distance across its frames), averaged across every bout -- how far a typical burst of movement covers, not just how long it lasts. `mean_immobile_bout_distance_<unit>` should come out near zero by construction; a large value there is a sign the mobility threshold/classification is off, not a real finding.
